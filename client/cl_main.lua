...
RegisterKeyMapping("openphone","Abrir Celular","keyboard", Adphone.OpenPhone)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if PhoneData.isOpen then
            ...
            for k, v in pairs(Adphone.ButtonDisable) do

                DisableControlAction(0, Adphone.ButtonDisable[k], true)

                DisableControlAction(1, Adphone.ButtonDisable[k], true)

                DisableControlAction(2, Adphone.ButtonDisable[k], true)
...
function OpenPhone()
    ...
    if PhoneData.isOpen or life <= Adphone.CheckLife or handcuffed then
    ...
    if HasPhone and checklicense then
        ...
        SendNUIMessage({
            ...
            action     = "UpdateAdphone",

            configfile = json.encode(Adphone.Client),

        })



        local languages = json.encode(Locales[Adphone.Locale])
...
function CallContact(CallData, AnonymousCall)
    ...
    for i = 1, Adphone.CallRepeats + 1, 1 do

        if not PhoneData.CallData.AnsweredCall then

            if RepeatCount + 1 ~= Adphone.CallRepeats + 1 then
                ...
                Citizen.Wait(Adphone.RepeatTimeout)
...
function CancelCall()
    ...
    if PhoneData.CallData.CallType == "ongoing" then
        ...
        if Adphone.CallSystem == "tokovoip" then

            exports.tokovoip_script:removePlayerFromRadio(PhoneData.CallData.CallId)

        elseif Adphone.CallSystem == "mumblevoip" then

            exports["mumble-voip"]:SetCallChannel(0)

        elseif Adphone.CallSystem == "saltychat" then

            local checksalty = vSERVER.SaltyChatEndCall(PhoneData.CallData)

        elseif Adphone.CallSystem == "pmavoice" then
...
RegisterNUICallback('GetHelpList', function(data, cb)
    ...
    local helplist = Adphone.HelpList
...
RegisterNUICallback('GetAdvertNews', function(data, cb)
	...
	local advert = Adphone.NewsAdvert
